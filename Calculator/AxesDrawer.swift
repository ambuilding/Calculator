...
class AxesDrawer {
    ...
    private func drawHashmarksInRect(bounds: CGRect, origin: CGPoint, pointsPerUnit: CGFloat) {
        ...
        if ((origin.x >= bounds.minX) && (origin.x <= bounds.maxX)) || ((origin.y >= bounds.minY) && (origin.y <= bounds.maxY)) {
            ...
            // now create a bounding box inside whose edges those four hashmark lie
    ...
    /*
     ...
     setting contectScaleFactor properly will enable us to put things on the closest pixel boundary
     if contentScaleactor is left to its default (1), then things will be on the nearest "point" boundary instead
